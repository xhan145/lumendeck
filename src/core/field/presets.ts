...
/**
 * The ten curated presets. Each binds a NON-EMPTY bundle to x, y AND z using real
 * render params: cfg/steps/denoise/seed on the sampler, strength on the Load
 * Image (img2img), scale/denoise/steps on Hires Fix, a mean control strength on
 * the ControlNet rack, a mean LoRA weight on the LoRA rack, and
 * motionStrength/frameCount/fps on the Video node.
 */
export const BUILTIN_FIELD_PRESETS: FieldPreset[] = [
  ...
  },
  {
    id: 'builtin-motion',
    name: 'Motion',
    description: 'Video node space: motion strength, frame count, and playback FPS.',
    builtin: true,
    axes: {
      x: { label: 'Motion', params: [P('video', 'motionStrength', 0, 2)] },
      y: { label: 'Frames', params: [P('video', 'frameCount', 2, 96)] },
      z: { label: 'FPS', params: [P('video', 'fps', 1, 30)] },
    },
  },
  {
...
/**
 * Flag preset params the CURRENT model ignores, so the UI can say "this axis
 * won't affect this render" instead of silently doing nothing (reuses the
 ...
 */
export function inertParamsForModel(preset: FieldPreset, modelId: string): string[] {
